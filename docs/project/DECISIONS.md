...
| 2026-04-18 | 간트 확장 렌더러 | 마일스톤형은 Mermaid Timeline 단일 preview만 사용하고, WBS형만 jsGanttImproved와 Mermaid adapter를 함께 사용한다. | milestone은 시점 중심 문서형 결과가 목적이고, WBS만 계층형 interactive schedule이 필요하기 때문이다. | Locked |
...
| 2026-04-18 | 간트 확장 렌더러 | milestone/WBS 모두 jsGanttImproved + Mermaid      | milestone은 Mermaid Timeline 단일 preview, WBS만 다중 adapter 유지 | milestone을 첫 번째 제대로 된 문서형 기능으로 다시 설계하기 위해서다. |
