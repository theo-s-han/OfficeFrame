...
| 2026-04-18 | 간트 확장 렌더러 | 마일스톤형과 WBS형은 jsGanttImproved와 Mermaid adapter를 사용한다.                                | milestone, hierarchy, document preview를 안정적으로 표현하기 위해서다.           | Locked |
...
| 2026-04-18 | 마일스톤 입력    | 마일스톤 `date`는 `YYYY-MM-DD`, status는 planned/on-track/done 스타일 정보로만 사용한다.          | milestone-only 데이터가 compact한 차트로 자연스럽게 보이도록 하기 위해서다.      | Locked |
