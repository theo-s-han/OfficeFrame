...
- 브랜드명은 `DataViz Studio`로 노출하고, 내부 개발용 문구는 사용자 화면에 남기지 않는다.
- 홈은 긴 랜딩 페이지보다 대표 기능을 바로 고를 수 있는 product hub처럼 보인다.
- 대표 기능 카드는 실제 결과물 느낌의 미리보기 이미지를 포함해, 어떤 시각화를 만드는 도구인지 한눈에 이해할 수 있다.
- 간트 차트와 마인드맵은 가장 먼저 보이는 1차 진입점으로 두고, 다른 대표 기능은 보조 카드로 노출한다.
- 홈 첫 화면 안에서 대표 기능 카드가 시작되어, 스크롤 전에도 선택 가능한 상태가 보인다.
...
- 단계 중심 입력을 사용하고, 기본 연결은 각 단계의 다음 단계/결정 분기에서 직접 정의한다.
- 단계 필드: 단계명, 단계 유형, lane(선택), 담당자, 입력값, 출력값, 설명.
- 결정 단계는 분기 라벨과 대상 단계를 2개 이상 입력한다.
- preview는 표준 shape와 분기 라벨, 단계별 입력/출력 요약을 함께 보여 준다.
- renderer는 HTML/CSS/SVG 기반 단일 preview surface를 사용한다.
