...
- WBS형 interactive preview: `jsgantt-improved`
  - `pGroup`, `pParent`, `pOpen`, `pDepend`, `setAdditionalHeaders`로 계층 구조와 일정 schedule 표현에 적합하다.
- 문서용 정적 preview: `mermaid`
  - milestone은 `timeline`, WBS는 `treeView-beta`, 발표용은 `mindmap` DSL을 제공한다.
...
- milestone은 문서형 preview 1개만 유지하고, WBS만 interactive/static preview를 분리한다.
...
| 마일스톤형  | 단일 날짜 milestone 흐름 관리   | Mermaid Timeline                           | name, date, section, previousStep, status, owner, notes                                              |
...
| section      | 아니오 | Timeline section                        |
| previousStep | 아니오 | 기존 milestone 목록에서 고르는 이전 단계 |
| status       | 아니오 | `planned`, `on-track`, `done`           |
| owner        | 아니오 | 담당자                                  |
| notes        | 아니오 | 상세 설명                               |

- `id`는 내부에서 자동 생성하고 사용자에게 노출하지 않는다.
- `previousStep` 선택값은 내부 `dependsOn[]` id reference로 변환한다.

### adapter 규칙

- Mermaid Timeline
  - `section` 기준으로 그룹화한다.
  - 날짜 기준으로 milestone을 정렬한다.
  - status는 구조 데이터가 아니라 label/badge용 텍스트로만 사용한다.
  - owner, notes는 기본 preview에서 최소화한다.
  - renderer 이름이나 DSL은 milestone 화면에 노출하지 않는다.
...
- 5~10일 간격
- 3~5개 section
- status는 row가 아니라 badge/text style로만 반영
